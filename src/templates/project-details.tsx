...
const Projects = ({ data }) => {
  // const { slug, stack, thumb, title } = project.frontmatter;
    ...
    <Layout>
      <Container>
        <ProjectWrap>
          ...
          <ProjectPreview>
            {/* <Image fluid={thumb.childrenImageSharp[0].fluid} /> */}
          </ProjectPreview>
          <ProjectSkills>
            <ul>
          ...
          <ProjectDetails>
            ...
            싶었는데, 다크모드와 새로운 탭에서 열기 기능을 구현해보면서 각종 컴포넌트의 상태를
            Context API를 이용해 전역으로 관리하는 흐름을 알게 되었습니다. 또한 페이지 단위가 아닌
            스크롤이 맨 아래로 내려갔을 때 새로운 데이터를 요청하는 무한스크롤 기능에 대한 이해가
            생겼으며, 백엔드와 함께한 프로젝트이기 때문에 REST API를 통한 요청(좋아요, 북마크 등)에
            대한 이해가 생겼습니다. 단순히 기능이 작동하게만 하는 것이 아니라 어떻게 하면 더 깔끔한
            코드를 작성할 수 있을지 고민하는 시간이 필요하며, 좋은 개발자로 성장하는 데 중요한
            것이라는 알게 되었습니다. 배포하는 과정에서 개발 과정에서는 보이지 않거나 별 것 아니라고
            생각하고 넘어갔던 것들로 인한 작은 오류들이 하나씩 발견되는 것을 보고 사전에 오류를 잡는
            것의 중요성을 느꼈고, 서비스의 규모가 커질수록 사소한 변경사항이라도 해결이 쉽지 않다는
            것을 느끼게 된 좋은 경험이었습니다. 배포 후에도 기능을 추가하거나 기존 버그를 수정하기
            위해 노력하고 있습니다.
          </ProjectDetails>
        </ProjectWrap>
      </Container>
...
const ProjectPreview = styled.figure``;
